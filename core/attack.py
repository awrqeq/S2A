...
class S2A_Final_Injector:
    def __init__(self, config):
        ...
        self.energy_ratio = self.s2a_config.get('energy_ratio_to_structure', 0.5)
        self.beta = self.s2a_config.get('blend_ratio_in_noise', 0.1)
        self.energy_threshold_struc = self.s2a_config.get('energy_threshold_struc', 0.85)
        self.energy_threshold_trigger = self.s2a_config.get('energy_threshold_trigger', 0.85)
    ...
    def _process_1d_col_wise(self, subband_data, L, energy_threshold):
        struc_t, noise_t = self._process_1d_row_wise(subband_data.T, L, energy_threshold)
    ...
    def _decompose_subband(self, subband_data, key, energy_threshold):
        if key == 'hl':
            return self._process_1d_row_wise(subband_data, self.L_1d, energy_threshold)
        elif key == 'lh':
            return self._process_1d_col_wise(subband_data, self.L_1d, energy_threshold)
        elif key == 'hh':
            return self._2d_ssa_decompose(subband_data, self.L_2d_h, self.L_2d_w, energy_threshold)
    ...
    def inject(self, img_tensor):
        ...
        for c in range(img_np.shape[0]):
            ...
            for key in self.subband_keys_to_attack:
                ...
                struc_clean, noise_clean = self._decompose_subband(subband_map_clean[key], key, self.energy_threshold_struc)
                energy_ratio = float(self.s2a_config.get('energy_ratio_to_structure', self.energy_ratio))
                norm_struc_clean = np.linalg.norm(struc_clean)
                norm_target = np.sqrt(energy_ratio) * (norm_struc_clean if norm_struc_clean > 1e-12 else 1.0)
                ...
                if self.use_constraint:
                    _, noise_from_poisoned = self._decompose_subband(subband_poisoned_preliminary, key, self.energy_threshold_struc)
            ...
            new_coeffs = [subband_map_final['ll'], (subband_map_final['hl'], subband_map_final['lh'], subband_map_final['hh'])]
