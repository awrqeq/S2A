# train_gtsrb_baseline.py
#
# 目的：使用与你后门攻击实验完全相同的配置，为GTSRB数据集训练一个干净的ResNet18基线模型。
#       这个脚本跑出的最终BA，就是你论文表格里需要的那个“Clean”数据。
#
# --- 核心配置 (完全复刻你的YAML) ---
# 1. 模型: 你的“智能适配版”ResNet18 (已传入 dataset_name='gtsrb')。
# 2. 数据增强: 与你 core/dataset.py 中为GTSRB配置的温和增强完全一致。
# 3. 优化器: SGD (lr=0.1, momentum=0.9, weight_decay=5e-4)。
...
class Config:
    ...
    # 其他
    SEED = 42
    IMAGE_SIZE = 64 # GTSRB 目标尺寸
    NUM_CLASSES = 43 # GTSRB 类别数
...
# --- 2. 模型定义 (从你的 core/models/resnet.py 完整复制，确保一致性) ---
...
# --- 3. 数据加载 (与你的 core/dataset.py 中为GTSRB配置的逻辑一致) ---
def get_dataloaders(batch_size, num_workers, image_size):
    # GTSRB 没有官方的均值/标准差，这里使用一个通用的
    mean = (0.5, 0.5, 0.5)
    std = (0.5, 0.5, 0.5)
    ...
    # 与你 core/dataset.py 中 GTSRB 训练变换一致
    ...
    testset = torchvision.datasets.GTSRB(root='./data', split='test', download=True, transform=transform_test)
...
def train(model, trainloader, optimizer, criterion, device):
    ...
    for inputs, targets in progress_bar:
        ...
        progress_bar.set_postfix(Loss=f"{loss.item():.3f}", Acc=f"{acc:.2f}%")
...
def main():
    ...
    try:
        trainloader, testloader = get_dataloaders(cfg.BATCH_SIZE, cfg.NUM_WORKERS, cfg.IMAGE_SIZE)
    except Exception as e:
        print(f"\n[错误] num_workers={cfg.NUM_WORKERS} 可能对你的系统过高。")
        print("尝试降低 num_workers 到 8 或 16。")
        print(f"详细错误: {e}")
        return

    # [!!!] 核心：创建模型时，传入类别数和数据集名称
    model = ResNet18(num_classes=cfg.NUM_CLASSES, dataset_name='gtsrb').to(device)
    ...
    print("--- 开始为GTSRB训练干净的ResNet18基线 (SGD + 余弦退火) ---")
    best_acc = 0.0
    for epoch in range(cfg.EPOCHS):
        ...
        if test_acc > best_acc:
            ...
            torch.save(model.state_dict(), 'gtsrb_resnet18_clean_baseline.pth')
